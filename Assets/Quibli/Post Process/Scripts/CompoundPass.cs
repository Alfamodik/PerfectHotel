...
public class CompoundPass : ScriptableRenderPass {
    ...
    public CompoundPass(InjectionPoint injectionPoint, List<CompoundRenderer> renderers) {
        ...
        m_Intermediate = new int[2];
        m_Intermediate[0] = Shader.PropertyToID("_IntermediateRT0");
        m_Intermediate[1] = Shader.PropertyToID("_IntermediateRT1");
    ...
    private RenderTargetIdentifier GetIntermediate(CommandBuffer cmd, int index) {
        if (!m_IntermediateAllocated[index]) {
            cmd.GetTemporaryRT(m_Intermediate[index], _intermediateDescriptor);
            m_IntermediateAllocated[index] = true;
        }

        return new RenderTargetIdentifier(m_Intermediate[index]);
    }
    ...
    private void CleanupIntermediate(CommandBuffer cmd) {
        for (int index = 0; index < 2; ++index) {
            if (m_IntermediateAllocated[index]) {
                cmd.ReleaseTemporaryRT(m_Intermediate[index]);
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData) {
        ...
        if (requireBlitBack) cmd.Blit(new RenderTargetIdentifier(m_Intermediate[0]), m_Destination);
